...
## Design options and strings

* How best to do this? a menu, a set of buttons on the main page?
** If we go with menu, when the page first launches (no current map), there's only a New Map button. When a current map exists, there is a "new" menu with items "New Map", and "New Pin" navigable by arrow keys, and with hotkeys. On-hover text is provided elsewhere.
** Or buttons on the main page instead of a menu? If so, then hide "New Pin" if there is no current map.
* In either case, n is mapped to New Map. a continues to be mapped to New Pin, quietly -- not documented. p is also mapped to New Pin and is the documented hotkey.
