...
Street data is fetched for a square region centered on the anchor pin, with a full edge length equal to the current Map Size setting (see [Settings](#settings)). This square is the map's data boundary — see [Pan Behavior](#pan-behavior) for what happens when panning reaches its edge.
...
### Major highway naming

Tier 1 ways (motorway/trunk) are the one case where TMAP prefers OSM's `ref` tag over its `name` tag: most people know a major highway by its numeric route designation (e.g. "I 80"), not its honorary/colloquial OSM name (e.g. "Eastshore Freeway"). At fetch time, a tier-1 way with a non-empty `ref` gets its display name replaced by that ref, humanized (see below); its original name, if any, is kept at `tags.honorificName` (not currently surfaced anywhere in the UI, kept as cheap insurance for a possible future use). A tier-1 way with no `ref` keeps its ordinary OSM name unchanged, same as any other tier.

A physical roadway can carry more than one concurrent route designation — OSM represents this as a single semicolon-separated `ref` value (e.g. `"I 80;I 580"` where two interstates share one alignment for a stretch). Every designation is kept, never just the first: they're comma-joined and sorted into numeric order (by each designation's own route number) so the same stretch of road reads identically regardless of which order OSM's data happens to store them in — adjacent segments of the same real-world concurrency aren't guaranteed to agree on order. The space between a designation's letter prefix and its number is also stripped (`"I 80"` → `"I80"`, `"CA 24"` → `"CA24"`) for braille readability and to keep a comma-joined multi-designation name unambiguous; nothing else about a designation's formatting is touched. The resulting name (e.g. `"I80,I580"`) flows through the ordinary [Feature name compacting](#feature-name-compacting) and [Label creation](#label-creation) pipeline unchanged, same as any street name — a route-number string has no stem/type structure for that pipeline to find, so it passes through as one undivided piece.

The fetch itself is also widened for this one case: a tier-1 way with a `ref` but no `name` at all — a real, if uncommon, OSM gap — is fetched and named from its ref rather than silently dropped by the ordinary "must have a name" filter. Ways of any other tier still require a name to be fetched.
...
Once the view is actually at the edge of the fetched data — the square region sized by the current Map Size setting around the anchor pin, see [Data sources](#data-sources) — a further pan in that direction is rejected: a tone plays (see [Sound cues](#sound-cues)) and the message display reports "Edge of Map."
...
**Letter sign (Grade 2 only):** a standalone single-letter word (e.g. a lettered grid street like "B Street") with no contraction of its own falls through to its plain letter cell — but for the letters that also serve as one of the 23 alphabetic wordsigns above, that plain cell is the *same* dot pattern as the wordsign's whole-word contraction (that's the whole mnemonic behind wordsigns — letter b's own shape doubles as "but," etc.). Note this never actually goes through the wordsign *rule* itself — that only matches a wordsign's full spelling (the literal word "but," three letters); a standalone single letter is always shorter than any wordsign's own text, so it can never match one there. The fallback just happens to produce the identical cell, since that's how wordsigns are defined in the first place. Because of that, the letter sign is needed **regardless of capitalization**, not just for a capitalized letter: this app's real content never has a standalone single letter meaning the actual grammatical word (that always appears fully spelled out, and gets contracted correctly by the normal wordsign rule instead — the word "but" spelled out still contracts normally); every standalone single letter it actually produces is a literal letter, so the letter sign is always correct there. dots 5,6 (the UEB letter sign), then the letter — plus the capital sign right after the letter sign if the letter was actually capitalized. A standalone capital "B" is `letter sign, capital sign, b`; a standalone lowercase "b" is `letter sign, b`. Both distinguish it from the wordsign "but," which is unaffected — spelling out the actual word "but" still contracts to its single cell exactly as before. Derived directly from the wordsign table itself (which letters' shapes are actually reused as a wordsign), not a separate hand-maintained list. Three letters — a, i, o — have no wordsign assigned to their shape at all, since each is already a genuine one-letter English word on its own ("a," "I," "o"); a standalone A, I, or O, either case, is therefore never ambiguous with anything and never gets the letter sign.
...
* **"Distance and Scale" heading**:
    ...
    * Map Size: the edge length of the square region fetched from Postpass around the anchor pin (see [Data sources](#data-sources)) — same two-independent-round-number-ladders pattern as Scale, switched by Units:
        * **Imperial**: [1 mile], 2 miles, 3 miles.
        * **Metric**: [1.5 km], 3 km, 5 km.
        * Index 0 (1 mile / 1.5 km) is the long-standing default and was, until this setting existed, a fixed constant — sized for the public Overpass instance's reliability limits at the time (a 1-mile-half-side/2mi-full-fetch reliably timed out for a dense test area). Larger presets only became viable once the live street-data source switched to Postpass (see [Data sources](#data-sources)), which measured well under 1 second for a full 3x3mi fetch in the same area. Unlike Scale, changing Map Size doesn't affect the map currently on screen — the data already fetched doesn't change size retroactively — it only takes effect on the *next* new map search (a fresh "New Map," not "New Pin," which joins the existing fetched square rather than re-fetching). Changing it announces "Map size: [value]. Applies to the next new map search." through the message field.
