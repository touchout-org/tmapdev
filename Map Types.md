...
**Scale range:** 1 mile to 50 miles to the inch. Picks up right where Street Maps' own ladder tops out (`SCALE_PRESETS_FT`'s last step is 5000 ft/inch, ≈0.95 mi/inch). Lowered from 100 mi/inch on 2026-08-09 -- production Postpass query timing against the original 600×400 mi box came in around 6-12s with real variance run to run (see project memory), so both the top scale step and the fetch box below were shrunk together to cut down element count and response-time variance, per the query performance note this doc originally flagged as untested.

**Fetch box:** 300 × 200 miles, fixed regardless of which scale within the range you're currently viewing -- sized so the full fetched area fits on the Dot Pad display (labels off) at the type's largest scale, 50 mi/inch (6.1875 in × 4.125 in display → ≈309 × 206 mi, rounded to the clean 300 × 200). Same 3:2 aspect ratio the display and the fetch box already share everywhere else in this app.
